        ...
        <Dialog static open={open} onClose={() => setOpen(false)} className="relative z-70">
          ...
          <div className="fixed inset-0 grid place-items-center p-5">
            <MotionDialogPanel
              ...
              className="w-[min(680px,100%)] rounded-[22px] border border-[#dedbd3] bg-white/85 p-2.5 shadow-[0_18px_50px_#00000024] backdrop-blur-[22px] dark:border-[#34322e] dark:bg-[#22211e]/85"
            >
              <div className="flex items-center gap-2.5 border-b border-[#dedbd3] px-2.5 pb-3 pt-2 dark:border-[#34322e]">
              ...
              <button
                className="relative mt-2 flex w-full items-center gap-2.5 rounded-[13px] border-0 bg-transparent p-2.5 text-left transition-colors hover:bg-[#e9eee6] dark:hover:bg-[#2a3029]"
                onClick={() => {
                  createNote();
                  setOpen(false);
                }}
              >
                ...
                Create note
                <kbd className="ml-auto text-xs text-[#6f6b64] dark:text-[#aaa39a]">⌘N</kbd>
              </button>
              {results.map((note) => (
                <button
                  key={note.id}
                  className="relative flex w-full items-center gap-2.5 rounded-[13px] border-0 bg-transparent p-2.5 text-left transition-colors hover:bg-[#e9eee6] dark:hover:bg-[#2a3029]"
                  onClick={() => {
                    openNote(note.id);
                    setOpen(false);
                  }}
                >
                  ...
                  {note.title}
                  <small className="ml-auto max-w-[42%] truncate text-[#6f6b64] dark:text-[#aaa39a]">{note.path}</small>
                </button>
              ))}
            </MotionDialogPanel>
          </div>
